...
def subseqs(s):
    ...
    [[]]
    """
    #the insight here is to add the first number to the subset of the base case 
    if not s:
        return [[]]
    else:
        subset = subseqs(s[1:])
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    """
    "*** YOUR CODE HERE ***"
    "the key insight here is num_trees(n) = ((4n - 6) * num_trees(n-1)) / n "
    if n == 1 or n == 2:
        return 1
...
class Keyboard:
    ...
    def typing(self, typing_input):
        ...
        for i in range(len(typing_input)):
            self.buttons[typing_input[i]].times_pressed += 1
            out_put += self.buttons[typing_input[i]].key
        return out_put
...
def insert(link, value, index):
    ...
    if index == 0:
        ...
        insert_into_all(link.rest, value, index - 1)
...
def long_paths(t, n):
    ...
    for b in t.branches:
        for path in long_paths(b, n - 1):
            return path_lst.append([t.label] + path)
    return path_lst
