...
def accumulate(combiner, base, n, term):
    ...
    """while i <= n:
        curr_value = term(i)
        total = combiner(total, curr_value)
        i += 1 
    return total
    """
...
def add_church(m, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    #first you have to define what is function two, three, four, five, etc. 
    # m -> lambda f: lambda x: f(f(x))
    # n -> lambda f: lambda x: f(f(f(x)))
    # ret -> lambda f: lambda x: f(f(f(f(f(x)))))
    # m(f)(x) apply m times functions on x 
